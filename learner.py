...
if __name__ == "__main__":
    ...
    half_life_seconds = Constants.TIME_HORIZON
    fps = 120 / frame_skip
    gamma = np.exp(np.log(0.5) / (fps * half_life_seconds))
    config = dict(
        actor_lr=1e-4,
        critic_lr=1e-4,
        n_steps=Constants.STEP_SIZE,
        batch_size=100_000,
    ...
    run_id = "rust_test_v0.01"
    wandb.login(key=os.environ["WANDB_KEY"])
    logger = wandb.init(dir="./wandb_store",
                        name="rust_test_v0.01",
                        project="rust_testing",
                        entity="kaiyotech",
                        id=run_id,
                        config=config,
    ...
    redis = Redis(username="user1", password=os.environ["redis_user1_key"],
                  db=Constants.DB_NUM)  # host="192.168.0.201",
    ...
    critic = Sequential(Linear(231, 256), LeakyReLU(), Linear(256, 256), LeakyReLU(),
                        Linear(256, 256), LeakyReLU(), Linear(256, 1))
    ...
    actor = DiscretePolicy(actor, shape=(90,))
    ...
    # alg.load("GP_saves/Opti_1691934607.8099136/Opti_58040/checkpoint.pt")
    ...
    alg.run(iterations_per_save=logger.config.save_every, save_dir="testing_saves")
